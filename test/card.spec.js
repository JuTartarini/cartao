...
describe('Retorna true or false para validar números de cartões de credito ', () => {
    ...
    describe('Cartões inválidos:', () => {
      ...
      it('Cartão inválido - 6062825624254005', () => {
        assert.equal(cardValidator.cardValidator('6062825624254005'), false);
        //expect(lib.cardValidator("5155901285384518")).to.be.equal(true);
      });
      it('Cartão válido com espaço no início - 6062825624254001', () => {
        assert.equal(cardValidator.cardValidator(' 6062825624254001'), false);
      });
      it('Cartão válido com espaço no fim 6062825624254001 ', () => {
        assert.equal(cardValidator.cardValidator('6062825624254001 '), false);
      });
      it('Cartão válido com espaço no meio 606282 5624254001', () => {
        assert.equal(cardValidator.cardValidator('606282 5624254001'), false);
      });
      it('Cartão válido com letras a6062825624254001', () => {
        assert.equal(cardValidator.cardValidator('a6062825624254001'), false);
      });
      it('Números inválidos - 0000111122223333', () => {
        assert.equal(cardValidator.cardValidator('0000111122223333'), false);
      });
      it('Letras - divirta-se', () => {
        assert.equal(cardValidator.cardValidator('divirta-se'), false);
      });
    });
  });
